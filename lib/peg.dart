 ...
 */
_Rule CHARCODE(spec, [name]) => spec is int ?
      new _CharCodeRule((code) => code == spec, name) :
      new _CharCodeRule(spec, name);
 ...
 */
_Rule LEX(arg1, [arg2]) => arg2 == null ?
    new _LexicalRule(arg1 is String ? arg1 : null, _compile(arg1)) :
    new _LexicalRule(arg1, _compile(arg2));
...
_Rule TEXT(rule, [extractor]) =>
    new _TextValueRule(_compile(rule), extractor == null ?
...
_Rule MANY0(rule, [separator = null]) =>
    new _RepeatRule(_compile(rule), _compileOptional(separator), 0);
...
class Symbol {
  ...
  String toString() => _rule == null ? '<$name>' : '<$name = $_rule>';
}
...
_Rule _compileOptional(rule) => rule == null ? null : _compile(rule);
...
class _EndOfInputRule extends _Rule {
  _match(_ParserState state, int pos) => pos == state._end ? [pos, null] : null;
...
class _CharCodeRule extends _Rule {
  ...
  _match(_ParserState state, int pos) {
    ...
    return _predicate(code) ? [pos + 1, null] : null;
  }

  String toString() => _name == null ? 'CHARCODE($_predicate)' : 'CHARCODE($_name)';
}
...
class _LexicalRule extends _Rule implements _Expectable {
  ...
  String description() => _name == null ? '?' : _name;
}

class _TextValueRule extends _Rule {
  final _Rule _rule;
  final Function _extract;

  _TextValueRule(this._rule, this._extract);
...
_Rule _compileMultiRule(List rules,
  ...
  for (var rule in rules) {
    if (reducer != null) throw new Exception('Reducer must be last in sequence: $rule');
    if (rule is Function) {
      if (allowReducer)
        reducer = rule;
      else
...
String _formatMultiRule(String functor, List rules) => '(' + rules.join(',') + ')';
...
class _SequenceRule extends _Rule {
  ...
  final bool _generatesValue;

  _SequenceRule(this._rules, int generatingSubRules, Function reducer)
      : _generatingSubRules = generatingSubRules,
        _reducer = reducer,
        _generatesValue = generatingSubRules > 0 || reducer != null;
  ...
  String toString() => _formatMultiRule('SEQ', _rules);
}
...
class _ChoiceRule extends _Rule {
  ...
  String toString() => _formatMultiRule('OR', _rules);
}

class _OptionalRule extends _Rule {
  final _Rule _rule;

  _OptionalRule(this._rule);
  ...
  String toString() => 'MAYBE($_rule)';
}

class _ContextRule extends _Rule {
  final _Rule _rule;

  _ContextRule(this._rule);

  _match(_ParserState state, int pos) {
    // TODO: protect error state.
    var match = _rule._match(state, pos);
    return  match == null ? null : [pos, null];
  }

  String toString() => 'AT($_rule)';
}

class _NegativeContextRule extends _Rule {
  final _Rule _rule;

  _NegativeContextRule(this._rule);

  _match(_ParserState state, int pos) {
    // TODO: protect error state.
    var match = _rule._match(state, pos);
    return match == null ? [pos, null] : null;
  }

  String toString() => 'NOT($_rule)';
}

class _RepeatRule extends _Rule {
  // Matches zero, one or more items.
  final _Rule _rule;
  final _Rule _separator;
  final int _min;
  ...
  _match(state, pos) {
    // First match.
    var match = _rule.match(state, pos);
    if (match == null) return _min == 0 ? [pos, []] : null;
    pos = match[0];
    var result = [match[1]];
  ...
  String toString() => 'MANY(min:$_min, $_rule${_separator==null?'':", sep: $_separator"})';
}
